...
type RestOf<T extends IDocument | NewDocument> = Omit<T, 'uploadDate' | 'issuingDate' | 'expirationDate'> & {
...
export class DocumentService {
  ...
  protected convertDateFromClient<T extends IDocument | NewDocument | PartialUpdateDocument>(document: T): RestOf<T> {
    return {
      ...
      issuingDate: document.issuingDate?.toJSON() ?? null,
      expirationDate: document.expirationDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDocument: RestDocument): IDocument {
    return {
      ...
      issuingDate: restDocument.issuingDate ? dayjs(restDocument.issuingDate) : undefined,
      expirationDate: restDocument.expirationDate ? dayjs(restDocument.expirationDate) : undefined,
    };
  }
