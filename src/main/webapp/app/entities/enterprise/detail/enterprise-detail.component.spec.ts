...
describe('Enterprise Management Detail Component', () => {
  ...
  beforeEach(async () => {
    await TestBed.configureTestingModule({
      imports: [EnterpriseDetailComponent],
      providers: [
        provideRouter(
          [
            {
  ...
  describe('OnInit', () => {
    it('Should load enterprise on init', async () => {
      const harness = await RouterTestingHarness.create();
      const instance = await harness.navigateByUrl('/', EnterpriseDetailComponent);

      // THEN
      expect(instance.enterprise()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
