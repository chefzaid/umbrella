...
import { EntityArrayResponseType, EnterpriseService } from '../service/enterprise.service';
...
export class EnterpriseComponent implements OnInit {
  ...
  trackId = (_index: number, item: IEnterprise): number => this.enterpriseService.getEnterpriseIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.enterprises = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IEnterprise[] | null): IEnterprise[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.enterpriseService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
