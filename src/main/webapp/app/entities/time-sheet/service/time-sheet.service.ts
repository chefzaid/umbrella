...
type RestOf<T extends ITimeSheet | NewTimeSheet> = Omit<T, 'creationDate' | 'submitDate' | 'validationDate'> & {
...
export class TimeSheetService {
  ...
  protected convertDateFromClient<T extends ITimeSheet | NewTimeSheet | PartialUpdateTimeSheet>(timeSheet: T): RestOf<T> {
    return {
      ...timeSheet,
      creationDate: timeSheet.creationDate?.format(DATE_FORMAT) ?? null,
      submitDate: timeSheet.submitDate?.toJSON() ?? null,
      validationDate: timeSheet.validationDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTimeSheet: RestTimeSheet): ITimeSheet {
    return {
      ...restTimeSheet,
      creationDate: restTimeSheet.creationDate ? dayjs(restTimeSheet.creationDate) : undefined,
      submitDate: restTimeSheet.submitDate ? dayjs(restTimeSheet.submitDate) : undefined,
      validationDate: restTimeSheet.validationDate ? dayjs(restTimeSheet.validationDate) : undefined,
    };
  }
