...
import formulaResolve from './formula-routing-resolve.service';

describe('Formula routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: FormulaService;
  let resultFormula: IFormula | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IFormula returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        formulaResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultFormula).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IFormula>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
