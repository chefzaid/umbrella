...
import { EntityArrayResponseType, FormulaService } from '../service/formula.service';
...
export class FormulaComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFormula): number => this.formulaService.getFormulaIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.formulas = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IFormula[] | null): IFormula[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.formulaService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
