...
/**
 * REST controller for managing {@link io.simpleit.umbrella.domain.Address}.
 */
@RestController
...
public class AddressResource {
    ...
    public ResponseEntity<List<Address>> getAllAddresses(
        ...
        if ("contact-is-null".equals(filter)) {
            ...
            return new ResponseEntity<>(addressService.findAllWhereContactIsNull(), HttpStatus.OK);
        }

        if ("employee-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where employee is null");
            return new ResponseEntity<>(addressService.findAllWhereEmployeeIsNull(), HttpStatus.OK);
        }

        if ("client-is-null".equals(filter)) {
            log.debug("REST request to get all Addresss where client is null");
            return new ResponseEntity<>(addressService.findAllWhereClientIsNull(), HttpStatus.OK);
        }
