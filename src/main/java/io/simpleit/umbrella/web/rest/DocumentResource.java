...
/**
 * REST controller for managing {@link io.simpleit.umbrella.domain.Document}.
 */
@RestController
...
public class DocumentResource {
    ...
    public ResponseEntity<List<Document>> getAllDocuments(
        ...
        if ("iddocument-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where idDocument is null");
            return new ResponseEntity<>(documentService.findAllWhereIdDocumentIsNull(), HttpStatus.OK);
        }

        if ("timesheet-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where timeSheet is null");
            return new ResponseEntity<>(documentService.findAllWhereTimeSheetIsNull(), HttpStatus.OK);
        }

        if ("expensenote-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where expenseNote is null");
            return new ResponseEntity<>(documentService.findAllWhereExpenseNoteIsNull(), HttpStatus.OK);
        }

        if ("invoice-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where invoice is null");
            return new ResponseEntity<>(documentService.findAllWhereInvoiceIsNull(), HttpStatus.OK);
        }

        if ("payslip-is-null".equals(filter)) {
            log.debug("REST request to get all Documents where paySlip is null");
            return new ResponseEntity<>(documentService.findAllWherePaySlipIsNull(), HttpStatus.OK);
        }
